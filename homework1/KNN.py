...
def KNNProcess():
    ...
    trainDocWordMap = {}         # 词典<key, value> key=cate_doc, value={{word1,tfidf1}, {word2, tfidf2},...}
    for line in open(trainFiles).readlines():
        ...
        for i in range(2, m, 2):  # 在每个文档向量中提取(word, tfidf)存入字典,索引从2 开始
            trainWordMap[lineSplit[i]] = lineSplit[i + 1]  # 字典内容：word—>TFIDF
        # 提取类别和文档名称
    ...
    kNNResultFile = 'TestSample/KNNClassifyResult1'
    testDocWordMap = {}
    for line in open(testFiles).readlines():
        ...
        for i in range(2, m, 2):
            testWordMap[lineSplit[i]] = lineSplit[i + 1]
